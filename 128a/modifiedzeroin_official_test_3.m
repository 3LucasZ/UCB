...
for j = 1:ntest
    ...
    %%%%
    [root, info] = modifiedzeroin3040268988(func, Int, params);
    profile off
...
final_score = sum(score_tot) / ntest / 5 + sum(score_extra);
disp("#####")
fprintf('Final score (reg + extra) : %.2f (%.2f + %.2f)  \n', final_score, sum(score_tot) / ntest / 5, sum(score_extra));