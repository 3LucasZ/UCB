...
class StudentUSocket(StudentUSocketBase):
  ...
  def connect(self, ip, port):
    ...
    packet = self.new_packet(ack=False, data=None, syn=True)
    self.tx(packet)
    self.state = SYN_SENT
  ...
  def handle_synsent (self, seg):
    ...
    if acceptable_ack:
      ...
      if self.snd.una |GT| self.snd.iss:
        # TODO: 1.3.4: Set your next sequence number to be the ack number for the ACK packet.
        self.state = ESTABLISHED
        self.set_pending_ack()
        self.update_window(seg=seg)
      ## End of Stage 1.3 ##
  ...
  def update_rto(self, acked_pkt):
    ...
    if self.rto == 1:
      ...
      self.rto = self.srtt + max(self.G, self.K * self.rttvar)
    self.rto = max(min(self.MAX_RTO, self.rto), self.MIN_RTO)
    ## End of Stage 9.1 ##
  ...
  def handle_accepted_seg(self, seg, payload):
    ...
    if self.state in [ESTABLISHED, FIN_WAIT_1, FIN_WAIT_2] and len(payload) > 0:
      self.handle_accepted_payload(payload=payload)
    ## End of Stage 2.2 ##
