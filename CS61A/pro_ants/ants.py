...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        while check.is_hive == False:
            if len(check.bees) > 0 and self.lower_bound <= space and space <= self.upper_bound:
...
# The WallAnt class
class WallAnt(Ant):
    """WallAnt has a lot of health to tank the colony."""
...
class TankAnt(ContainerAnt):
    """TankAnt protects ants and attacks bees."""
...
class ScubaThrower(ThrowerAnt):
    """Regular thrower that can be placed on water."""
...
class QueenAnt(ThrowerAnt):
    ...
    def action(self, gamestate):
        ...
        # attack
        super().action(gamestate)
        # double all damage of ants behind
        cur = self.place.exit
...
class SlowThrower(ThrowerAnt):
    ...
    def throw_at(self, target):
        ...
        "*** YOUR CODE HERE ***"
        if target is not None:
            regular_action = target.action
            def slowed_action(gamestate):
                if target.slowed_turns == 0:
                    target.action = regular_action
                    regular_action(gamestate)
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self):
        # BEGIN Problem EC 4
        ret = {}
        tar, dist = self.place, 0
        while tar is not None:
            for bee in tar.bees:
                ret[bee] = dist
            if tar.ant is not None and tar.ant is not self:
                ret[tar.ant] = dist
            if tar.ant is not None and tar.ant.is_container and tar.ant.ant_contained is not None and tar.ant.ant_contained is not self:
                ret[tar.ant.ant_contained] = dist
            tar, dist = tar.entrance, dist + 1
        return ret
        # END Problem EC 4
...
class Bee(Insect):
    ...
    #Custom constructor w/ slowed_turns
    def __init__(self, health):
        super().__init__(health)
        #slow
        self.slowed_turns = 0
