...
def pick(paragraphs, select, k):
    ...
    """
    # BEGIN PROBLEM 1
    target_paragraphs = [p for p in paragraphs if select(p)]
    if k >= len(target_paragraphs):
        return ''
    else:
        return target_paragraphs[k]
    # END PROBLEM 1
...
def about(subject):
    ...
    # BEGIN PROBLEM 2
    def relevant(paragraph):
        words = split(paragraph)
        words = [remove_punctuation(lower(word)) for word in words]
        print('DEBUG:',words)
        for special in subject:
            if special in words:
                return True
        return False
    return relevant
...
def memo_diff(diff_function):
    ...
    def memoized(typed, source, limit):
        ...
        if key in cache.keys():
            ...
            if limit <= val[1]:
                return val[0] 
        val = (diff_function(typed, source, limit), limit)
        cache[key] = val
        return val[0]
        # END PROBLEM EC
    return memoized
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    """
    # BEGIN PROBLEM 5
    if len(word_list) == 0 or typed_word in word_list:
        return typed_word
    best_word, lowest_cost = word_list[0], diff_function(typed_word,word_list[0],limit)
    for tar_word in word_list:
        cost = diff_function(typed_word,tar_word,limit)
        if cost < lowest_cost:
            best_word = tar_word
            lowest_cost = cost
    if lowest_cost <= limit:
        return best_word
    else:
        return typed_word
    # END PROBLEM 5
...
def furry_fixes(typed, source, limit):
    ...
    if len(typed) == 0:
        ...
        return len(typed)
    errors = 0 if typed[0] == source[0] else 1
    return errors + furry_fixes(typed[1:], source[1:], limit-errors)
    # END PROBLEM 6
...
def minimum_mewtations(typed, source, limit):
    ...
    if typed[0] == source[0]:
        ...
        return 1 + min(minimum_mewtations(typed,source[1:],limit-1), #append
                       minimum_mewtations(typed[1:],source[1:],limit-1), #replace
                       minimum_mewtations(typed[1:],source,limit-1), #delete
                       )
...
def time_per_word(words, timestamps_per_player):
    ...
    """
    # BEGIN PROBLEM 9
    transform = lambda lst: [lst[i+1]-lst[i] for i in range(len(lst)-1)]
    return words, [transform(player) for player in timestamps_per_player]
    # END PROBLEM 9
...
def fastest_words(match_object):
    ...
    for i in word_indices:
        ...
        for j in player_indices:
            if get_time(match_object, j, i) < get_time(match_object, best_player, i):
                best_player = j
        ret[best_player].append(get_word(match_object,i))
    return ret
    # END PROBLEM 11
