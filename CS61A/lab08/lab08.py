...
def add_links(link1, link2):
    ...
    def clone(link):
        if link.rest == Link.empty:
            rest_clone_end = Link(link.first)
            return rest_clone_end, rest_clone_end 
        else:
            rest_clone, rest_clone_end = clone(link.rest)
            return Link(link.first, rest_clone), rest_clone_end
    lnk1, lnk1_end = clone(link1)
    lnk2, lnk2_end = clone(link2)
    lnk1_end.rest = lnk2
    return lnk1
