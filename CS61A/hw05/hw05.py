    ...
    def edit_gene(gene, mul):
        while gene is not Link.empty:
            if gene.first == 'AUG':
                ...
                return
            gene = gene.rest
    def helper(lnk_of_genes, mul):
        if lnk_of_genes is Link.empty:
            return
        edit_gene(lnk_of_genes.first, mul)
        helper(lnk_of_genes.rest, mul + 1)
    helper(lnk_of_genes, 1)
...
def transcribe(dna):
    ...
    return [] if len(dna) == 0 else [dict[dna[0]]+dict[dna[1]]+dict[dna[2]]] + transcribe(dna[3:])
